...
{
	Super::BeginPlay();
	boxComp->OnComponentBeginOverlap.AddDynamic(this, &AItemUltimate::OnOverlap);
...
// Called every frame
void AItemUltimate::Tick(float DeltaTime)
{
...
void AItemUltimate::OnOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	APlayerFlight* player = Cast<APlayerFlight>(OtherActor);

	if (player != nullptr && !player->GetIsDead())
	{
		UE_LOG(LogTemp, Warning, TEXT("ultimate +1"));
		Destroy();
	}
